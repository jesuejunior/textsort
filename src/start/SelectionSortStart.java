...
public class SelectionSortStart {
	...
	public static void main(String[] args) throws IOException {
		...
		long start = System.currentTimeMillis(); //Tempo antes imediatamente o inicio da ordenação
		...
		long end = System.currentTimeMillis(); //Tempo apos imediatamente finalizar a ordenação
		now = new Date(end); 
		...
		for (int i = 0; i < fileSorted.length; i++) {
			...
			DoFile.save("out.txt", fileSorted[i] + "\n", true); //out.txt é o nome do arquivo a ser salvo
		}
	}
